...
{
	return cuda::std::floor2(T(0)) == T(0)
	   &&  cuda::std::floor2(T(1)) == T(1)
	   &&  cuda::std::floor2(T(2)) == T(2)
	   &&  cuda::std::floor2(T(3)) == T(2)
	   &&  cuda::std::floor2(T(4)) == T(4)
	   &&  cuda::std::floor2(T(5)) == T(4)
	   &&  cuda::std::floor2(T(6)) == T(4)
	   &&  cuda::std::floor2(T(7)) == T(4)
	   &&  cuda::std::floor2(T(8)) == T(8)
	   &&  cuda::std::floor2(T(9)) == T(8)
	   ;
}
...
void runtime_test()
{
	ASSERT_SAME_TYPE(T, decltype(cuda::std::floor2(T(0))));
	ASSERT_NOEXCEPT(             cuda::std::floor2(T(0)));
	
	assert( cuda::std::floor2(T(121)) == T(64));
	assert( cuda::std::floor2(T(122)) == T(64));
	assert( cuda::std::floor2(T(123)) == T(64));
	assert( cuda::std::floor2(T(124)) == T(64));
	assert( cuda::std::floor2(T(125)) == T(64));
	assert( cuda::std::floor2(T(126)) == T(64));
	assert( cuda::std::floor2(T(127)) == T(64));
	assert( cuda::std::floor2(T(128)) == T(128));
	assert( cuda::std::floor2(T(129)) == T(128));
	assert( cuda::std::floor2(T(130)) == T(128));
}
...
int main()
{
    ...
    {
    auto lambda = [](auto x) -> decltype(cuda::std::floor2(x)) {};
	...
	__uint128_t val = 128;
	val <<= 32;
	assert( cuda::std::floor2(val-1) == val/2);
	assert( cuda::std::floor2(val)   == val);
	assert( cuda::std::floor2(val+1) == val);
	val <<= 2;
	assert( cuda::std::floor2(val-1) == val/2);
	assert( cuda::std::floor2(val)   == val);
	assert( cuda::std::floor2(val+1) == val);
