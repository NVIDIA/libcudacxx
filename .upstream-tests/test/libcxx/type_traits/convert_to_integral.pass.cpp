...
{
  typedef cuda::std::numeric_limits<From> Limits;
  const From max = Limits::max();
  const From min = Limits::min();
  {
  auto ret = cuda::std::__convert_to_integral((From)max);
  assert(ret == max);
  ret = cuda::std::__convert_to_integral((From)min);
  assert(ret == min);
  static_assert(cuda::std::is_same<decltype(ret), To>::value, "");
  }
  {
  UserDefinedIntegral<From> f(max);
  auto ret = cuda::std::__convert_to_integral(f);
  assert(ret == max);
  f.value = min;
  ret = cuda::std::__convert_to_integral(f);
  assert(ret == min);
  static_assert(cuda::std::is_same<decltype(ret), To>::value, "");
  }
  {
  ...
  auto ret = cuda::std::__convert_to_integral(e);
  assert(ret == max);
  e = static_cast<Enum>(min);
  ret = cuda::std::__convert_to_integral(min);
  assert(ret == min);
  static_assert(cuda::std::is_same<decltype(ret), To>::value, "");
  }
}
...
{
  auto ret = cuda::std::__convert_to_integral((From)1);
  assert(ret == 1);
  static_assert(cuda::std::is_same<decltype(ret), To>::value, "");
}
...
enum enum2 : unsigned long {
  value = cuda::std::numeric_limits<unsigned long>::max()
};
...
int main(int, char**)
{
  ...
  typedef cuda::std::underlying_type<enum1>::type Enum1UT;
  check_enum_types<enum1, decltype(((Enum1UT)1) + 1)>();
  typedef cuda::std::underlying_type<enum2>::type Enum2UT;
