...
void do_search(Iter1 b1, Iter1 e1, Iter2 b2, Iter2 e2, Iter1 result, unsigned max_count) {
    cuda::std::boyer_moore_horspool_searcher<Iter2,
           typename cuda::std::hash<typename cuda::std::remove_cv<typename cuda::std::iterator_traits<Iter2>::value_type>::type>, count_equal> s{b2, e2};
    count_equal::count = 0;
    assert(result == cuda::std::search(b1, e1, s));
